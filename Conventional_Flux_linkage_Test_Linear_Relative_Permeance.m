%% Flux linkage Test Based on Relative Permeance  (Conventional PMSM)
% This program is designed to test the flux linkage in the Conventional PMSM
% based on the linear relative permeance model
...
for Ns = Ns_range
    ...
    for Pr = Pr_range
        ...
        for Ratio_of_tooth = Ratio_of_tooth_range
            ...
            %% calculate the flux linkage in RP model
            ...
            t = parameters_of_other_part.period_of_machine;
             if (mod(Ns, Np*t)==0 && Ps<Ns)
                parameters_of_performance.flux_linkage = Flux_linkage_calculation_based_on_linear_relative_permeance(parameters_of_rotor, parameters_of_stator, parameters_of_other_part, parameters_of_space_harmonics, parameters_of_time_harmonics);
            else
                parameters_of_performance.flux_linkage =0;
             end
             
            %% save the data in the struct of parameters
            n = n+1;
            RP.motor(n) =  Record_parameters_of_motor_relative_permeance(parameters_of_rotor, parameters_of_stator, parameters_of_other_part, parameters_of_space_harmonics, parameters_of_time_harmonics, parameters_of_performance);   
        end
    end
end
toc
...
table = struct2table(RP.motor);
% use the core parameters of the table;
RP_table = table(:,{'number_of_slot', 'pole_pairs_of_stator', 'pole_pairs_of_rotor', 'ratio_of_tooth', 'flux_linkage'});
% save the table in the txt docoment
writetable(RP_table, 'RP_table.txt');
